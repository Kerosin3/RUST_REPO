...
fn main() {
    ...
    // imitator
    // let imitator_thrd = thread::spawn(move || imitate_termo_change(_dev1_handler));
    // imitator_thrd.join().unwrap();
...
fn imitate_termo_change(handle: Device_Handler) {
    let mut rng = rand::thread_rng();
    loop {
        handle.property_change_state(rng.gen_range(10..50)).unwrap();
