...
impl SmartHouse {
    ...
    pub fn get_dev_report(&self, a_device: &Rc<RefCell<dyn Device>>) -> Option<String> {
        ...
        let dev_name = a_device.borrow().get_name().clone();
    ...
    fn delete_room(&mut self, a_room: &str) -> Result<(usize, usize), ErrorC> {
        if let Some(n_room) = self.test_whether_room_exists(a_room) {
    ...
    pub fn append_dev_to_a_room(
        ...
        if let Some(room_pos) = self.test_whether_room_exists(a_room) {
            if let Ok(_) = self.rooms.get(room_pos).unwrap().add_device(a_device) {
                Ok(Device_Handler::new(a_device))
            } else {
                Err(ErrorC::DeviceInRoomExists(
                    a_device.borrow().get_name().clone(),
                ))
            }
        } else {
            Err(ErrorC::RoomNotExists(a_room.to_string()))
...
impl RoomObj for Room_Generic {
    ...
    fn add_device(&self, some_dev: &Rc<RefCell<dyn Device>>) -> Result<(), ErrorC> {
        if let Some(_) = self.find_dev_name(some_dev.borrow().get_name().as_ref()) {
            Err(ErrorC::DeviceInRoomExists(
                some_dev.borrow().get_name().clone(),
            ))
        } else {
...
impl Device_Handler {
    ...
    fn delete_device(&self) -> Result<(), ErrorC> {
        if let Some(handle) = self.dev.upgrade() {
            unimplemented!()
        } else {
            Err(ErrorC::ErrorHandleOperation)
        }
    }
}
...
mod tests {
    ...
    fn test_get_devices_in_room() {
        ...
        let rez_handler = sh.append_dev_to_a_room("room1", &dev);
        let rez_handler1 = sh.append_dev_to_a_room("room1", &dev1);
